...
void DisplayUtils::printCalender(int originDayOfWeek, int month, int currentDate, int year) {
      ...
      GLCD.CursorToXY((i%CALENDER_NO_COLS) *  (CALENDER_DATE_SEPARATOR + 2* DIGIT_FONT_WIDTH) + ((i-originDayOfWeek) < 9?DIGIT_FONT_WIDTH:0) +1, 
        ...
        if (currentDate == (i  - originDayOfWeek + 1)) {
         ...
         GLCD.DrawRect((i%CALENDER_NO_COLS) *  (CALENDER_DATE_SEPARATOR + 2* DIGIT_FONT_WIDTH) + ((i-originDayOfWeek) < 9?DIGIT_FONT_WIDTH:0) -2,
          ...
          GLCD.DrawRect((i%CALENDER_NO_COLS) *  (CALENDER_DATE_SEPARATOR + 2* DIGIT_FONT_WIDTH) + ((i-originDayOfWeek) < 9?DIGIT_FONT_WIDTH:0) -0,
          VERTICAL_OFFSET + (i/7) * (  DIGIT_FONT_HEIGHT + CALENDER_LINE_SEPARATOR) -2, 
          9, 8);
        }
...
void DisplayUtils::displayClockPageOne() {
  ...
  if ((currTime.hr24 == 0) && (currTime.mn == 0) && ( currTime.ss <= 2))
  {
    GLCD.ClearScreen();
    createLayout();
    DisplayUtils::printCalender(currTime.firstDayOfMonth, currTime.month, currTime.day, currTime.year);
    printHumidity(92, 2);
    printPressure(92, 11);
  }
